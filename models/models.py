...
from utils import general, metric
from networks import networks
from objectives import ncc, dmmr
from objectives import regularizers


import importlib.util
module_path = "/vol/alan/users/toz/midir-pycharm/model/loss.py"
module_name = "loss_module"
spec = importlib.util.spec_from_file_location(module_name, module_path)
loss_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(loss_module)

MILossGaussian = loss_module.MILossGaussian
...
class ImplicitRegistrator:
    ...
    def __init__(self, moving_image, fixed_image, **kwargs):
        ...
        elif self.loss_function_arg.lower() == "mi":
            self.criterion = MILossGaussian(normalised=False)

        elif self.loss_function_arg.lower() == "nmi":
            self.criterion = MILossGaussian()

        elif self.loss_function_arg.lower() == "dmmr":
            self.criterion = dmmr.DMMRLoss()
        ...
        self.fixed_seg_mask = kwargs['target_seg']
        self.moving_seg_mask = kwargs['source_seg']
        self.metric_history = []
    ...
    def set_default_arguments(self):
        ...
        self.args["target_seg"] = None
        self.args["source_seg"] = None
    ...
    def fit(self, epochs=None, red_blue=False):
        ...
        for i in tqdm.tqdm(range(epochs)):
            ...
            if i % self.log_interval == 0:
                self.measure_metrics(i)

    def make_coordinate_tensor(self, shape):
        """Create a coordinate tensor."""
        return general.make_coordinate_tensor(shape).cpu()

    def preprocess_output(self, output, image_shape):
        """Preprocess the output tensor."""
        displacement_prediction = output.unsqueeze(1).cpu().detach().numpy()
        displacement_prediction = displacement_prediction.reshape(1, 3, *image_shape)
        return displacement_prediction

    def preprocess_transformed_image(self, transformed_image, image_shape):
        """Preprocess the transformed image."""
        transformed_prediction = transformed_image.detach().numpy()
        transformed_prediction = transformed_prediction.reshape(image_shape)
        transformed_prediction = np.expand_dims(transformed_prediction, axis=(0, 1))
        return transformed_prediction

    def preprocess_segmentation(self, segmentation):
        """Preprocess the segmentation."""
        processed_segmentation = sitk.GetArrayFromImage(segmentation)
        processed_segmentation = np.expand_dims(processed_segmentation, axis=(0, 1))
        return processed_segmentation

    def preprocess_segmentation_tensor(self, output, coord_tensor, segmentation):
        """Preprocess the source segmentation tensor."""
        transformed_segmentation = self.transform(output.cpu(), coord_tensor.cpu(),
                                                  torch.tensor(segmentation).squeeze().cpu())
        processed_segmentation = transformed_segmentation.detach().numpy()
        processed_segmentation = processed_segmentation.reshape(self.moving_image.shape)
        processed_segmentation = np.expand_dims(processed_segmentation, axis=(0, 1))
        return processed_segmentation

    def measure_metrics(self, epoch):
        """Measure metrics."""
        moving_coord_tensor = self.make_coordinate_tensor(self.moving_image.shape)

        net = self.network.cpu()
        output = net(moving_coord_tensor)
        moving_image = self.moving_image.clone()
        transformed_moving_image = self.transform(output, moving_coord_tensor, moving_image.cpu())

        disp_pred = self.preprocess_output(output, self.moving_image.shape)
        target_pred = self.preprocess_transformed_image(transformed_moving_image, self.moving_image.shape)
        target = np.expand_dims(self.fixed_image.cpu().detach().numpy(), axis=(0, 1))
        target_seg = self.preprocess_segmentation(self.fixed_seg_mask)
        source_seg = self.preprocess_segmentation(self.moving_seg_mask)
        warped_source_seg = self.preprocess_segmentation_tensor(output, moving_coord_tensor, source_seg)
        ...
        metric_results = metric.measure_metrics(metric_data,
                                                metric_groups=["disp_metrics", "image_metrics", "seg_metrics"])

        self.metric_history.append({**metric_results, 'epoch': epoch})
        self.network.cuda()
