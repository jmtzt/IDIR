...
def load_image_BraTS(fixed_id, moving_id, data_dir, mask_dir, mod):
    if mod == 't1t2':
        moving_path = f'{data_dir}/BraTS20_Training_{moving_id:03}/BraTS20_Training_{moving_id:03}_t2.nii.gz'
        moving_mask_path = f'{mask_dir}/BraTS20_Training_{moving_id:03}/mask_m2_BraTS20_Training_{moving_id:03}.nii.gz'
    else:  # 't1t1
        moving_path = f'{data_dir}/BraTS20_Training_{moving_id:03}/BraTS20_Training_{moving_id:03}_t1.nii.gz'
        moving_mask_path = f'{mask_dir}/BraTS20_Training_{moving_id:03}/mask_m1_BraTS20_Training_{moving_id:03}.nii.gz'

    _fixed = sitk.ReadImage(f'{data_dir}/BraTS20_Training_{fixed_id:03}/BraTS20_Training_{fixed_id:03}_t1.nii.gz')
    _fixed_mask = sitk.ReadImage(f'{mask_dir}/BraTS20_Training_{fixed_id:03}/'
                                 f'mask_m1_BraTS20_Training_{fixed_id:03}.nii.gz')
    _moving = sitk.ReadImage(moving_path)
    _moving_mask = sitk.ReadImage(moving_mask_path)

    dtype = torch.float32

    fixed_image = torch.tensor(sitk.GetArrayFromImage(_fixed), dtype=dtype)
    moving_image = torch.tensor(sitk.GetArrayFromImage(_moving), dtype=dtype)

    fixed_mask = np.clip(sitk.GetArrayFromImage(_fixed_mask), 0, 1)
    moving_mask = np.clip(sitk.GetArrayFromImage(_moving_mask), 0, 1)

    return fixed_image, moving_image, fixed_mask, moving_mask, _fixed, _moving


def load_image_camcan(fixed_id, moving_id, data_dir, mask_dir, mod):
    if mod == 't1t2':
        moving_path = f'{data_dir}/sub-{moving_id}/T2_brain.nii.gz'
        moving_mask_path = f'{mask_dir}/sub-{moving_id}/mask_m2_sub-{moving_id}.nii.gz'
    else:  # 't1t1
        moving_path = f'{data_dir}/BraTS20_Training_{moving_id}/T1_brain.nii.gz'
        moving_mask_path = f'{mask_dir}/sub-{moving_id}/mask_m1_sub-{moving_id}.nii.gz'

    fixed_seg_path = f'{data_dir}/sub-{fixed_id}/T1_brain_mask.nii.gz'
    fixed_seg_mask = sitk.ReadImage(fixed_seg_path)

    moving_seg_path = f'{data_dir}/sub-{moving_id}/T1_brain_mask.nii.gz'
    moving_seg_mask = sitk.ReadImage(moving_seg_path)

    _fixed = sitk.ReadImage(f'{data_dir}/sub-{fixed_id}/T1_brain.nii.gz')
    _fixed_mask = sitk.ReadImage(f'{mask_dir}/sub-{fixed_id}/'
                                 f'mask_m1_sub-{fixed_id}.nii.gz')
    _moving = sitk.ReadImage(moving_path)
    _moving_mask = sitk.ReadImage(moving_mask_path)
    ...
    return fixed_image, moving_image, fixed_mask, moving_mask, _fixed, _moving, fixed_seg_mask, moving_seg_mask
